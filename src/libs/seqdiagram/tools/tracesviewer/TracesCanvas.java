...
public class TracesCanvas
	...
	public HashMap<String, Integer> actors = new HashMap<String, Integer>();
	public HashMap<String, Arrow> arrows = new HashMap<String, Arrow>();
	public HashMap<String, Color> arrowsColors = new HashMap<String, Color>();
	...
	public void refreshTracesCanvas(TracesSession tracesSession,
		String tracesOrigin) {

	    if (this.tracesSession != null && this.tracesOrigin != null &&
	            this.tracesSession.equals(tracesSession) && this.tracesOrigin.equals(tracesOrigin)) {
	        // nothing to change
	        return;
	    }
	...
	public void drawTop(Graphics g) {
		...
		while (e.hasMoreElements()) {
			...
			Image actorsImage = Actor.getImage(origin);
			if (actorsImage != null) {
		...
		try {
			...
			for (int i = 0; i < tracesSession.size(); i++) {
				...
				if (actors.get(from) == null) {
					actors.put(
						from,
						new Integer(sizeActors * HORIZONTAL_GAP + FIRST_ACTOR_GAP));
				}
	...
	public void constructArrows() {
		...
		for (int i = 0; i < tracesSession.size(); i++) {
			TracesMessage tracesMessage =
				tracesSession.elementAt(i);

			from = tracesMessage.getFrom();
			to = tracesMessage.getTo();

			int positionXFrom = actors.get(from).intValue();
			int positionXTo = actors.get(to).intValue();
			...
			if (transactionId != null) {
				color = arrowsColors.get(transactionId);
				if (color == null)
					color = Color.black;
			} else
				color = Color.black;
	...
	public void assignColors() {
		arrowsColors = new HashMap<String, Color>();
...
// PC 2.0 add test for traceViewer set to null
					tracesViewer != null) {
			...
			oldArrow = arrows.get(selectedArrowName);
	...
	public void setBackground(Graphics g) {
		...
		if (backgroundImage != null
			...
			g.fillRect(0, ACTORS_STRIPE-20, getSize().width, getSize().height);
		}
	...
	public void paint(Graphics g) {
		try {
	        ...
	        while (e.hasMoreElements()) {
	            ...
	            // Vertical line
	            g.setColor(Color.black);
	            g.drawLine(positionX, ACTORS_STRIPE, positionX, getSize().height);
	        }
